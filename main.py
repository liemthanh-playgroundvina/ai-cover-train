...
def run_train_script(
    ...
    # My param
    save_every_epoch = str(int(total_epoch) + 1)
    detector = 0
    ...
    try:
        ...
        model_dir = os.path.join(current_directory, "logs")
        pattern = rf"{model_name}_{total_epoch}e_(\d+)s\.pth"
        ...
        for filename in os.listdir(model_dir):
            ...
            if match:
                ...
                if value > max_value:
                    ...
                    max_file = filename
        print(max_file)
        path_model = os.path.join(model_dir, max_file)

        index_dir = os.path.join(model_dir, model_name)
        pattern_index = r"added_(.*)\.index"
        for filename in os.listdir(index_dir):
            if re.match(pattern_index, filename):
                index_file = filename
                break
        path_index = os.path.join(index_dir, index_file)
    except:
        raise Exception("Could not find model, and index file. Maybe training failed.")
